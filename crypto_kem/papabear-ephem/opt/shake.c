/* Optimized cSHAKE-256 using Keccak from KCP */
#include "shake.h"
#include "keccakf1600.h"

#define HASH_STATE_BYTES (1600/8)
#define CSHAKE256_RATE (HASH_STATE_BYTES-256/4)
#define CSHAKE256_PAD 0x04 /* SHAKE would be 0x1F */

static void xorbytes(uint64_t *ctx64, const uint8_t *in, unsigned position, unsigned len) {
    KeccakF1600_StateXORBytes(ctx64,in,position,len);
}

static void keccakf(hash_ctx_t ctx) {
    KeccakF1600_StatePermute(ctx->state.w);
    ctx->position = 0;
}

void hash_update (
    struct hash_ctx_s * __restrict__ ctx,
    const uint8_t *in,
    size_t len
) {
    assert(!ctx->squeezing);
    while (len >= (unsigned)(CSHAKE256_RATE - ctx->position)) {
        size_t cando = CSHAKE256_RATE - ctx->position;
        xorbytes(ctx->state.w, in, ctx->position, cando);
        keccakf(ctx);
        len -= cando;
        in  += cando;
    }
    xorbytes(ctx->state.w, in, ctx->position, len);
    ctx->position += len;
}

void hash_output (
    hash_ctx_t ctx,
    uint8_t * __restrict__ out,
    size_t len
) {
    if (!ctx->squeezing) {
        if (ctx->position >= CSHAKE256_RATE) keccakf(ctx);
        ctx->squeezing = 1;
        uint8_t tmp = CSHAKE256_PAD;
        xorbytes(ctx->state.w,&tmp,ctx->position,1);
        tmp = 0x80;
        xorbytes(ctx->state.w,&tmp,CSHAKE256_RATE-1,1);
        keccakf(ctx);
    }
    
    while (len >= (unsigned)(CSHAKE256_RATE - ctx->position)) {
        size_t cando = CSHAKE256_RATE - ctx->position;
        KeccakF1600_StateExtractBytes(ctx->state.w,out,ctx->position,cando);
        len -= cando;
        out += cando;
        keccakf(ctx);
    }
    KeccakF1600_StateExtractBytes(ctx->state.w,out,ctx->position,len);
    ctx->position += len;
}

static inline void hash_update_byte (hash_ctx_t ctx, uint8_t b) {
    assert(!ctx->squeezing);
    if (ctx->position >= CSHAKE256_RATE) keccakf(ctx);
    xorbytes(ctx->state.w,&b,ctx->position++,1);
}

/* Hash instances for ThreeBears */
void threebears_cshake_init(hash_ctx_t ctx) {
    const uint8_t precomputed[HASH_STATE_BYTES] = {
        
    };
    memset(ctx,0,sizeof(hash_ctx_t));
    xorbytes(ctx->state.w,precomputed,0,sizeof(precomputed));
    ctx->position = ctx->squeezing = 0;
}

void hash_times_n (
    uint8_t *output,
    unsigned outlen,
    unsigned stride,
    const hash_ctx_t ctx0,
    uint8_t iv,
    unsigned n
) {
    for (unsigned i=0; i<n; i++) {
        hash_ctx_t ctx;
        memcpy(ctx,ctx0,sizeof(ctx));
        hash_update_byte(ctx,iv+i);
        hash_output(ctx,output+i*stride,outlen);
        hash_destroy(ctx);
    }
}
