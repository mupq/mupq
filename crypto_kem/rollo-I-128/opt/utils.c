...
u2 Shift_32bits_5Words_Left_4bits(pu4 pu4X)
{
    ...
    u4 u4CarryMSB[5] = { ((*(pu4X    )) & 0xF0000000) >> 28,
...
u1 byte_to_word(pu4 v_destination, pu1 v_initial, u1 nb_coeff)
{
  ...
  {
    for(int i=0; i<PARAM_M_LEN_BYTES-2; i=i+4)
    {
      word = *(v_initial+i+ PARAM_M_LEN_BYTES*j) ^ (*(v_initial+i+1+PARAM_M_LEN_BYTES*j)<<8) ^ (*(v_initial+i+2+PARAM_M_LEN_BYTES*j)<<16) ^ (*(v_initial+i+3+PARAM_M_LEN_BYTES*j)<<24);
      *(v_destination++) = word;

    }
    word = *(v_initial+PARAM_M_LEN_BYTES-2+ PARAM_M_LEN_BYTES*j) ^ (*(v_initial+PARAM_M_LEN_BYTES-2+1+PARAM_M_LEN_BYTES*j)<<8);
    *(v_destination++) = word;
  }
  return 0;
