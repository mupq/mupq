...
void ffi_poly_resize(ffi_poly p, uint64_t size) {
  ...
  ffi_vec_set(tmp, p->v, min(p->degree+1, (int64_t)size+1));
 ...
 */
static void plain_mul(ffi_poly o, const ffi_poly a, const ffi_poly b) {
 ...
 * \param[in] b Polynomial
 */
static void kar_fold(ffi_vec res, ffi_vec src, int max, int half_size) {
...
static void kar_mul(ffi_vec o, ffi_vec a, ffi_vec b, int a_size, int b_size, ffi_vec stack) {
...
void ffi_poly_mul(ffi_poly o, const ffi_poly a, const ffi_poly b) {
  ...
  if((int64_t)a->size < max_degree+1) ffi_poly_resize(a, max_degree+1);
  if((int64_t)b->size < max_degree+1) ffi_poly_resize(b, max_degree+1);
  if((int64_t)o->size < 2*(max_degree+1)) ffi_poly_resize(o, 2*(max_degree+1));
...
void ffi_poly_mulmod(ffi_poly o, const ffi_poly a, const ffi_poly b, const ffi_poly modulus) {
  if((int64_t)o->size < modulus->degree) {
...
void ffi_poly_mulmod_sparse(ffi_poly o, const ffi_poly p1, const ffi_poly p2, const ffi_poly_sparse modulus) {
  ...
  for(int i = unreduced->degree - modulus_degree ; i >= 0 ; i--) {
    for(int j = 0 ; j < (int)modulus->coeffs_nb-1 ; j++) {
...
void ffi_poly_div(ffi_poly q, ffi_poly r, const ffi_poly a, const ffi_poly b) {
  if((int64_t)q->size < a->degree+1) {
    ffi_poly_resize(q, a->degree+1);
  }

  if((int64_t)r->size < a->degree+1) {
