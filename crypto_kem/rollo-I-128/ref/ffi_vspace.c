...
uint64_t ffi_vspace_intersection(ffi_vspace o, const ffi_vspace a, const ffi_vspace b, uint64_t a_size, uint64_t b_size) {
  ...
  for(size_t i = 0 ; i < a_size + b_size ; ++i) {
    ...
    for(size_t j = pivot+1 ; j < a_size + b_size ; ++j) {
    ...
    for(size_t j = i+1 ; j < a_size + b_size ; ++j) {
      if(!(ffi_elt_is_zero(direct_sum[j]) || (int)ffi_elt_get_degree(direct_sum[j]) < degree)) {
  ...
  for(size_t i = 0 ; (int)i < inter_dim ; i++) {
