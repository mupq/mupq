...
uint32_t rbc_vec_gauss(rbc_vec v, uint32_t size) {
  ...
  for(size_t i = 0 ; i < size ; ++i) {
    ...
    while(t[degree] != -1) {
      rbc_elt_add(v[i], v[i], v[t[degree]]);
      degree = rbc_elt_get_degree(v[i]);
      if(degree == -1) break;
    }

    if(rbc_elt_is_zero(v[i]) != 1) {
      t[degree] = i;
    }
  }

  // Row echelon form reconstruction
  rbc_vec v2;
  rbc_vec_init(&v2, size);

  for(int32_t j = RBC_FIELD_M - 1 ; j >= 0 ; j--) {
    if(t[j] != -1) {
      rbc_elt_set(v2[rank], v[t[j]]);
      rank++;
    }
  }

  rbc_vec_set(v, v2, size);
  rbc_vec_clear(v2);
...
uint32_t rbc_vec_get_rank(const rbc_vec v, uint32_t size) {
  ...
  for(size_t i = 0 ; i < size ; ++i) {
    degree = rbc_elt_get_degree(copy[i]);

    while(degree != -1 && t[degree] != -1) {
      rbc_elt_add(copy[i], copy[i], copy[t[degree]]);
      degree = rbc_elt_get_degree(copy[i]);
    }

    if(rbc_elt_is_zero(copy[i]) != 1) {
      t[degree] = i;
    }
  }
