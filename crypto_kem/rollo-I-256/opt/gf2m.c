...
const u4 MODULO_GF2_M[4] = {0x00000201, 0x00000000, 0x00000000, 0x00020000};
...
u2 Add_GF2m(pu4 pu4X, pu4 pu4Y, pu4 pu4R, u2 u1Len)
{
    int i;
...
u2 Mupliply_GF2m_PreCompute(pu4 pu4X, pu4 pu4Storage, u1 u1Len)
{
    ...
    Add_GF2m(pu4Storage +  u1Len, pu4Storage +   2*u1Len, pu4Storage +  3*u1Len, 4);      // 0x3
    Shift_32bits_Words_Left(pu4X, pu4Storage +   4*u1Len, u1Len, 2);                      // 0x4
    Add_GF2m(pu4Storage +  4*u1Len, pu4Storage +   u1Len, pu4Storage +  5*u1Len, 4);      // 0x5
    Add_GF2m(pu4Storage +  4*u1Len, pu4Storage + 2*u1Len, pu4Storage +  6*u1Len, 4);      // 0x6
    Add_GF2m(pu4Storage +    u1Len, pu4Storage + 2*u1Len, pu4Storage +  7*u1Len, 4);      // 0x7
    Add_GF2m(pu4Storage +  4*u1Len, pu4Storage + 7*u1Len, pu4Storage + 7*u1Len, 4);       // 0x7
    Shift_32bits_Words_Left(pu4X, pu4Storage +   8*u1Len, u1Len, 3);                      // 0x8
    Add_GF2m(pu4Storage +  8*u1Len, pu4Storage +   u1Len, pu4Storage +  9*u1Len, 4);      // 0x9
    Add_GF2m(pu4Storage +  8*u1Len, pu4Storage +   2*u1Len, pu4Storage +  10*u1Len, 4);   // 0xA
    Add_GF2m(pu4Storage +  8*u1Len, pu4Storage +   2*u1Len, pu4Storage +  11*u1Len, 4);   // 0xB
    Add_GF2m(pu4Storage + 11*u1Len, pu4Storage +   u1Len, pu4Storage +  11*u1Len, 4);     // 0xB
    Add_GF2m(pu4Storage +  8*u1Len, pu4Storage +  4*u1Len, pu4Storage +  12*u1Len, 4);    // 0xC
    Add_GF2m(pu4Storage +  8*u1Len, pu4Storage +  4*u1Len, pu4Storage +  13*u1Len, 4);    // 0xD
    Add_GF2m(pu4Storage +    u1Len, pu4Storage +  13*u1Len, pu4Storage +  13*u1Len, 4);   // 0xD
    Add_GF2m(pu4Storage +  4*u1Len, pu4Storage +  8*u1Len, pu4Storage +  14*u1Len, 4);    // 0xE
    Add_GF2m(pu4Storage +  2*u1Len, pu4Storage +  14*u1Len, pu4Storage +  14*u1Len, 4);   // 0xE
    Add_GF2m(pu4Storage +  2*u1Len, pu4Storage +   u1Len, pu4Storage +  15*u1Len, 4);     // 0xF
    Add_GF2m(pu4Storage +  4*u1Len, pu4Storage +  15*u1Len, pu4Storage +  15*u1Len, 4);   // 0xF
    Add_GF2m(pu4Storage +  8*u1Len, pu4Storage +  15*u1Len, pu4Storage +  15*u1Len, 4);   // 0xF
 ...
 * \brief This function efficiently performs the modular reduction modulo P = X^113 + X^9 + 1
...
u2 Modular_Reduction_GF2m(pu4 pu4X, pu4 pu4R)
{
  ...
  *(pu4R + 3) ^= u4MSBStorage[10] ^ u4MSBStorage[11]  ^ u4MSBStorage[16];
  *(pu4R + 2) ^= u4MSBStorage[6] ^ u4MSBStorage[7]  ^ u4MSBStorage[8] ^ u4MSBStorage[9];
  *(pu4R + 1) ^= u4MSBStorage[2] ^ u4MSBStorage[3] ^ u4MSBStorage[4] ^ u4MSBStorage[5] ^ u4MSBStorage[14] ^ u4MSBStorage[15] ^ u4MSBStorage[19];
  * pu4R      ^= u4MSBStorage[0] ^ u4MSBStorage[1] ^ u4MSBStorage[12] ^  u4MSBStorage[13] ^  u4MSBStorage[17]^  u4MSBStorage[18];

    // Extract bits 17 to 31 of C[3]
  u4 test  = *(pu4R + 3) >> 17;
  * pu4R  ^= test;
  *pu4R   ^= test << 9;
