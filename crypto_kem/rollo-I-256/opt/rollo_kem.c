...
/**
 * \fn static u1 GenKey( _SecretKey *SecretKey, _PublicKey *PublicKey, pu4 pu4Workspace)
...
static u1 GenKey( _SecretKey *SecretKey, _PublicKey *PublicKey, pu4 pu4Workspace)
{
  u1 u1a_inv_degree;

  pu4 pu4Sk_a_copy = pu4Workspace;
  pu4 pu4a_inv = pu4Sk_a_copy + NM_WORDS;
  pu4 pu4modulo = pu4Sk_a_copy + NM_WORDS;
  ...
  u1a_inv_degree  = Degree_GF2mn( pu4a_inv,PARAM_M_LEN_WORD, PARAM_N);
  u1 nbcoeff_a_inv = u1a_inv_degree + 1;

  // Step 4 : Inverse moving to gain memory usage
  memcpy(pu4Sk_a_copy, pu4a_inv, nbcoeff_a_inv*PARAM_M_LEN_WORD*4);
  memset(pu4a_inv, 0x00, 2*NM_WORDS*4);
  pu4a_inv = pu4Sk_a_copy ;
  ...
  pu4 pu4R_Karatsuba = pu4a_inv + (pad_Karatsuba+1)*PARAM_M_LEN_WORD;
  ...
  ModularReduction(PublicKey->pu4Pk,
                   2*PARAM_N,
                   PARAM_N,
                   PARAM_M_LEN_WORD);

  //The workspace is cleared
  memset(pu4Workspace,0x00, (2*NM_WORDS + 3*pad_Karatsuba*PARAM_M_LEN_WORD)*4);
...
/**
 * \fn static pu4 Encaps(_PublicKey PublicKey, pu4 pu4Cipher, pu1 ss, pu4 pu4Workspace, pu1 pk, pu1 e2, pu1 e1, pu1 E)
...
static pu4 Encaps(_PublicKey PublicKey, pu4 pu4Cipher, pu1 ss, pu4 pu4Workspace)
{
  ...
  pu4Error      = pu4Support + PARAM_M_LEN_WORD* PARAM_N; //(PARAM_R + 5) * PARAM_M_LEN_WORD;
  Create_gf2mn_using_support(pu4Error, pu4Support, PARAM_R);

  // Step 3 : Multiplication between PublicKey.pu4Pk and pu4Error (pk*e_1)
  u1 pad_Karatsuba = Karatsuba_Degree(PARAM_N);
  pu4BufferCalculus = pu4Error + (pad_Karatsuba+1)*PARAM_M_LEN_WORD;

  Karatsuba(pu4Error,
            PublicKey.pu4Pk,
            pu4BufferCalculus,
            pu4BufferCalculus + 2*pad_Karatsuba*PARAM_M_LEN_WORD,
            pad_Karatsuba,
            PARAM_M_LEN_WORD);

  memset(pu4BufferCalculus + 2*pad_Karatsuba * PARAM_M_LEN_WORD,
         0x00,
         pad_Karatsuba * PARAM_M_LEN_WORD*4);
  ...
  // Step 4 : Generation of the second part of the error (e_2) from pu4Support,
  // it replaces the first part of the error to gain memory usage
  memset(pu4Error, 0x00, PARAM_N*PARAM_M_LEN_WORD*4);
  ...
  pu1 support = (pu1) (pu4Support + PARAM_M_LEN_WORD*PARAM_R);
...
static u2 Decaps(_SecretKey SecretKey, pu4 pu4Cipher, pu1 ss, pu4 pu4Workspace)
{
  ...
  u1 pad_Karatsuba = Karatsuba_Degree(PARAM_N);
  pu4 pu4Workspace_tmp = pu4Syndrome + 2* pad_Karatsuba*PARAM_M_LEN_WORD;
  ...
  memset(pu4Workspace_tmp,
         0x00,
         3*pad_Karatsuba*PARAM_M_LEN_WORD*4);
  ...
  pu4Workspace_tmp= pu4Syndrome + NM_WORDS;
...
int crypto_kem_dec(unsigned char* ss, unsigned char* ct, unsigned char* sk)
{
  ...
  Decaps(*(_SecretKey *)sk, (pu4)ct, ss, workspace);
