...
u2 Shift_32bits_5Words_Left_4bits(pu4 pu4X)
{
    ...
    *(pu4X + 6) <<= 4;  *(pu4X + 6) ^= u4CarryMSB[5];
    *(pu4X + 7) <<= 4;  *(pu4X + 7) ^= u4CarryMSB[6];
...
u1 word_to_byte(pu1 v_destination, pu4 v_initial, u1 len_v, u1 nb_coeff)
{
  ...
  for(int j=0;j<nb_coeff;j++)
  {
    ...
    *(v_destination++) = byte;
    byte = (*(v_initial+len_v-1+PARAM_M_LEN_WORD*j)>>16 )& 0xff;
    *(v_destination++) = byte;
  }
  return 0;
