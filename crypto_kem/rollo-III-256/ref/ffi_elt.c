...
static uint64_t SQR_LOOKUP_TABLE[256];
...
void ffi_elt_inv(ffi_elt o, const ffi_elt e) {
  ...
  while(u[0] != 1) {
    ...
    if(j == 0) {
      for(size_t i = 0; i < FIELD_UINT64; ++i){
        u[i] ^= v[i];
        g1[i] ^= g2[i];
      }
    }
...
void ffi_elt_ur_mul(ffi_elt_ur o, const ffi_elt e1, const ffi_elt e2) {
  ...
  for(size_t i = 2 ; i < 16 ; i += 2) {
    ...
    for(size_t j = 1 ; j < size_Bu ; ++j) {
      carry = Bu[i>>1][j-1] >> 63;
      Bu[i][j] = (Bu[i>>1][j] << 1) ^ carry;
    }

    for(size_t j = 0 ; j < size_Bu ; ++j) {
  ...
  for(int k = 64/w - 1 ; k > 0 ; --k) {
    for(size_t j = 0 ; j < FIELD_UINT64 ; ++j) {
      u = (e1[j] >> (w*k)) & 0xF;
      for(size_t i = 0 ; i < size_Bu ; ++i) {
        o[j+i] ^= Bu[u][i];
      }
    }

    for(size_t i = 2*FIELD_UINT64-1 ; i > 0 ; --i) {
      o[i] = (o[i] << 4) ^ (o[i-1] >> 60);
    }
