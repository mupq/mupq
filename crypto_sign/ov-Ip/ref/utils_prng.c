...
#define ov_aes128_ctr_keyexp_publicinputs aes128_ctr_keyexp_publicinputs
...
static void aes_ctr(unsigned char *out, size_t outlen, const unsigned char *iv, uint32_t ctr, const uint64_t *rkeys, unsigned int nrounds) {
    uint32_t ivw[16];
    size_t i;
    uint32_t cc = ctr;
...
static void ov_aes128_ctr_publicinputs(unsigned char *out, size_t outlen, const unsigned char *iv, uint32_t ctr, const aes128ctx_publicinputs *ctx) {
    ...
    aes_ctr(out, outlen, iv, ctr, (uint64_t *)*(ctx->sk_exp), 10);
