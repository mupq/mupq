...
EXPORT void sdith_rng_create_xof4_ctx(XOF4_CTX *ctx, void **in, int inBytes) {
  ...
  shake128_inc_init(&ctx->states[0]);
  shake128_inc_init(&ctx->states[1]);
  shake128_inc_init(&ctx->states[2]);
  shake128_inc_init(&ctx->states[3]);
#else
  Keccak_HashInitializetimes4_SHAKE256(&xof4_ctx);
#endif
  //Keccak_HashUpdatetimes4(&xof4_ctx, (const uint8_t **)in, inBytes << 3);
  shake128_inc_absorb(&ctx->states[0], in[0], inBytes);
  shake128_inc_absorb(&ctx->states[1], in[1], inBytes);
  shake128_inc_absorb(&ctx->states[2], in[2], inBytes);
  shake128_inc_absorb(&ctx->states[3], in[3], inBytes);

  //Keccak_HashFinaltimes4(&xof4_ctx, NULL);
  shake128_inc_finalize(&ctx->states[0]);
  shake128_inc_finalize(&ctx->states[1]);
  shake128_inc_finalize(&ctx->states[2]);
  shake128_inc_finalize(&ctx->states[3]);
}
...
EXPORT void sdith_xof4_next_bytes(XOF4_CTX *ctx, void **out, int outLen) {
  ...
  //                          outLen << 3);
  shake128_inc_squeeze(out[0], outLen, &ctx->states[0]);
  shake128_inc_squeeze(out[1], outLen, &ctx->states[1]);
  shake128_inc_squeeze(out[2], outLen, &ctx->states[2]);
  shake128_inc_squeeze(out[3], outLen, &ctx->states[3]);
}
